...
def do_run(base_directory, input_file_name, output_file_name, manager):
    ...
    out_text = "ID;ProjectName;RepoID;DependencyProjectName;DependencyRequirements;DependencyProjectID\n"
    ...
    with open(base_directory + input_file_name, encoding="utf-8", errors='replace') as infile:
        for line in infile:
            ...
            try:
                if tokens[4] == manager or tokens[4] == manager.upper() or tokens[4] == manager.lower():
                    try:
                        out_file.write(tokens[0] + ";" + tokens[2] + ";" + tokens[3] + ";" + tokens[9] + ";" + tokens[10] + ";" + tokens[12] + "\n")
                        _found += 1
